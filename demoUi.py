...
class MainWindow(QtGui.QMainWindow):
    ...
    signalMoney = QtCore.pyqtSignal(type(Event()))
    ...
    def registerEvent(self):
        ...
        self.signalMoney.connect(self.showMoney)
        ...
        self.__eventEngine.register(EVENT_ACCOUNT, self.signalMoney.emit)
    ...
    def updateInvestor(self, event):
        ...
        self.setWindowTitle(u'CTP交易终端 ' + data['InvestorName'].decode('GBK'))

    #----------------------------------------------------------------------
    def showMoney(self, event):
        """"""
        log = u'净值:%.2f 可用: %.2f '%(event.dict_['data']['Balance'],event.dict_['data']['Available'])
